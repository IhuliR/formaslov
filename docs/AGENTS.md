...
Сейчас `Label` глобальная модель без `user`.

При любых изменениях labels сначала выбрать доменную модель:

1. Глобальный справочник меток для всех пользователей.
2. Пользовательские метки, где каждая метка принадлежит конкретному user.

Нельзя случайно сделать половинчатое состояние.
...
6. `Label` глобальная, без user ownership.
7. `IsAuthor` не подходит напрямую для `Annotation`.
8. Создание аннотации требует усиления ownership validation.
9. `start/end` аннотаций требуют явной валидации.
10. `Annotation.text` ограничен `max_length=500`.
11. Редактирование документа может ломать offsets аннотаций.
12. `backend/static/schema.yaml` нужно синхронизировать с фактическим API.
13. Старые templates `document.html` и `login.html` похожи на legacy и требуют проверки перед удалением.
